...
// 动画片段定义（帧数范围）
// 注意：这些帧数需要根据实际的 Lottie JSON 文件调整
const animationSegments = {
    idle: { start: 0, end: 119, loop: true },           // 待机（循环）
    walkSouth: { start: 120, end: 149, loop: true },    // 向南走（循环）
    walkNorth: { start: 150, end: 179, loop: true },    // 向北走（循环）
    walkWest: { start: 180, end: 209, loop: true },     // 向西走（循环）
    walkEast: { start: 210, end: 239, loop: true },     // 向东走（循环）
    pickup: { start: 240, end: 280, loop: false },       // 拾取（播放一次）
    eat: { start: 281, end: 330, loop: false }          // 吃东西（播放一次）
};
...
function initCharacterAnimation() {
    ...
    // 注意：这里需要替换为实际的 Lottie JSON 文件路径
    // 如果还没有动画文件，可以先使用占位符或注释掉
    const animationPath = 'animations/character.json'; // 替换为实际路径
    ...
    try {
        ...
        console.warn('⚠️ Lottie 动画初始化失败（可能是动画文件不存在）:', error);
        // 如果动画文件不存在，显示占位符
...
function playCharacterAnimation(actionName) {
    ...
    if (segment.loop) {
        ...
        // 当到达末尾时，重新播放该片段
        characterAnimation.addEventListener('enterFrame', function loopHandler() {
            if (characterAnimation.currentFrame >= segment.end) {
        ...
        // 非循环动画，播放完成后自动返回待机
        characterAnimation.addEventListener('complete', function completeHandler() {
            characterAnimation.removeEventListener('complete', completeHandler);
            playCharacterAnimation('idle');
        });
    }
