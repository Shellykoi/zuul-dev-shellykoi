...
// 动画片段定义（帧数范围）
// 根据实际的 Lottie JSON 文件：只有一个行走循环（0-30帧，30fps）
const animationSegments = {
    idle: { start: 0, end: 30, loop: true },            // 待机（使用行走动画的第一帧）
    walkSouth: { start: 0, end: 30, loop: true },       // 向南走（循环，0度旋转）
    walkNorth: { start: 0, end: 30, loop: true },       // 向北走（循环，180度旋转）
    walkWest: { start: 0, end: 30, loop: true },        // 向西走（循环，90度旋转）
    walkEast: { start: 0, end: 30, loop: true },        // 向东走（循环，-90度旋转）
    pickup: { start: 0, end: 30, loop: false },         // 拾取（播放一次，不旋转）
    eat: { start: 0, end: 30, loop: false }             // 吃东西（播放一次，不旋转）
};
...
function initCharacterAnimation() {
    ...
    try {
        ...
        // 如果动画文件不存在，显示占位符
        container.innerHTML = '<div style="color: var(--text-medium); text-align: center; padding: 20px; font-size: 0.9rem;">🎮<br>角色动画<br><small>（动画加载失败）</small></div>';
...
// 存储当前动画状态
let currentAnimationName = null; // 初始为null，等待动画加载完成后再设置为idle
...
function playCharacterAnimation(actionName) {
    ...
    if (container) {
        // 根据方向应用旋转
        if (actionName === 'walkNorth') {
            container.style.transform = 'rotate(180deg)';
        } else if (actionName === 'walkWest') {
            container.style.transform = 'rotate(90deg)';
        } else if (actionName === 'walkEast') {
            container.style.transform = 'rotate(-90deg)';
        } else if (actionName === 'walkSouth') {
            container.style.transform = 'rotate(0deg)';
        } else {
            // idle, pickup, eat 不旋转，保持默认方向（南）
            container.style.transform = 'rotate(0deg)';
        }
    }
