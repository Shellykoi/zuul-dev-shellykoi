...
function initCharacterAnimation() {
    ...
    // 注意：动画文件暂时注释掉，因为文件不存在
    // 如果以后有动画文件，可以取消注释并设置正确的路径
    // const animationPath = 'animations/character.json';
    
    // 暂时不加载动画，避免404错误
    // 如果以后有动画文件，可以取消注释下面的代码
    /*
    try {
        const animationPath = 'animations/character.json';
    ...
    */
    ...
    // 暂时不加载动画，显示占位符
    characterAnimation = null; // 设置为null，避免后续代码报错
    container.innerHTML = '<div style="color: var(--text-medium); text-align: center; padding: 40px;">🎮<br>角色动画<br><small>（动画文件暂未配置）</small></div>';
}
